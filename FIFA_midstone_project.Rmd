...
#########This notebook was used as scratch work to make my shiny app. its a bit unorganized 
and somewhat random ###########################
...
#saveRDS(la_liga_table, file = "data/la_liga_table.rds")
```
...
saveRDS(player_data, file = "data/fifa_player_data_07-17.rds")
```
```{r}
dim(player_data)
```
...
find_new_pct <- function(team, year){
  ...
  if( !(team %in% previous_year_team)) {
   ...
   B <- df_percentage %>%
    ...
    return ((length(difference)/length(A)) * 100)
...
percent_colums_added


```
```{r}
ggscatter(percent_colums_added, x = "Overall_average", y = "win_percentage", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "Team Rating Average", ylab = "Win Percentage")+
  theme(legend.position='none')+
   font("xlab", size = 16)+
 font("ylab", size = 16)
```
...
```{r}
ggplot(percent_colums_added, aes(x = Overall_average, y = win_percentage))+ geom_point()
```

```{r}
ggscatter(percent_colums_added, x = "new_player_pct", y = "win_percentage", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "Percent New Players", ylab = "Win Percentage")+
  theme(legend.position='none')+
   font("xlab", size = 16)+
 font("ylab", size = 16)
```
...
```{r}
ggplot(percent_colums_added, aes(x = new_player_pct, y = win_percentage))+ geom_point()
```

```{r}
one_team <-percent_colums_added %>% filter(club == 'FC Barcelona')
...
```{r}
model2 <- lm(win_percentage ~ Overall_average , data = percent_colums_added)
summary(model2)
```

```{r}
model <- lm(win_percentage ~ Overall_average + new_player_pct , data = percent_colums_added)
summary(model)
...
```{r}
find_new_players(year = 2016, team = 'Real Madrid')
```
```{r}
...
#   
#   for (x in unique_team){
#     
#     player_unique <- find_new_players(year = i, team = x)
#    print(player_unique)
#   }
#   
# }
...
#library(d3Tree)
random_tree  <- sunburst_chart%>%group_by(club,Nationality, Name, Age,Position)%>%count()
...
```{r}
percent <- with_players %>% 
  mutate (win_percentage = round(matches_won/total_matches *100)) %>%
  #mutate (lose_percentage = round((matches_lost/total_matches)*100)) %>% 
  #mutate (draw_percentage = round((matches_drawn/total_matches)*100)) %>% 
  select(season,club, matches_won,matches_lost,matches_drawn) %>% 
  group_by(season, club) %>% 
...
lm_df <-with_players %>%
  group_by(season, club) %>%
  mutate(win_percentage = (matches_won/total_matches)*100,
         Overall_average = mean(Overall)) %>% 
  ungroup() %>% 
  select(season,club,win_percentage,Overall_average,Overall)
...
```{r}
#library(glmnet)
model1 <- lm(win_percentage ~ Overall_average, data = lm_df)
summary(model1)
#plot(model1)
```
